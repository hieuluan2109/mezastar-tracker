      ...
      <div className="relative w-full max-w-2xl rounded-2xl bg-panel border border-primary/80 shadow-2xl p-6 md:p-8 animate-in fade-in zoom-in-95 duration-200 theme-transition">
        ...
        <div className="grid gap-6 md:grid-cols-12 grid-cols-1 mt-2">
          ...
          <div className="md:col-span-5 flex flex-col items-center">
            <div className={`relative flex aspect-[7/10] w-full max-w-[200px] items-center justify-center rounded-2xl bg-root border p-3 ${
              ...
              {/* Placeholder */}
              <div className={`flex flex-col items-center justify-center text-center p-4 rounded-xl text-secondary ${tag.imageUrl ? 'hidden' : ''}`}>
          ...
          <div className="md:col-span-7 flex flex-col justify-between space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap gap-2 items-center">
                <span className={`text-[10px] px-2 py-0.5 rounded font-black uppercase tracking-wider ${rarityBadgeColor}`}>
                  {rarityLabel}
                </span>
                <span className={`text-[10px] px-2.5 py-0.5 rounded font-bold ${typeBadgeClass}`}>
                  Hệ {tag.type}
                </span>
              </div>
            ...
            <div className="bg-root/60 p-4 rounded-xl border border-primary/80 flex items-center justify-between">
              <div>
                <span className="text-xs text-secondary block font-semibold">Số lượng đang sở hữu:</span>
                <span className="text-lg font-black text-primary">{quantity} thẻ</span>
              </div>
              <div className="flex items-center space-x-1.5">
                <button
                  onClick={() => onQuantityChange(tag.id, -1)}
                  disabled={quantity <= 0}
                  className="h-9 w-9 rounded-lg bg-elevated text-secondary hover:text-primary hover:bg-muted disabled:opacity-30 disabled:hover:bg-elevated flex items-center justify-center font-bold text-lg transition-colors"
                >
                  -
                </button>
                <button
                  onClick={() => onQuantityChange(tag.id, 1)}
                  className="h-9 w-9 rounded-lg bg-elevated text-secondary hover:text-primary hover:bg-muted flex items-center justify-center font-bold text-lg transition-colors"
                >
                  +
                </button>
              </div>
            ...
            <div className="space-y-1.5">
              <label htmlFor="detail-notes" className="text-xs font-bold text-secondary flex items-center space-x-1">
                <MapPin className="h-3.5 w-3.5 text-rose-500" />
                <span>Ghi chú sưu tập (Sự kiện, Nơi quay được, Ngày săn...):</span>
              </label>
              <textarea
                id="detail-notes"
                value={localNote}
            ...
            <div className="flex items-center justify-between pt-2">
              <button
                onClick={handleSave}
                className="inline-flex items-center space-x-2 bg-rose-600 hover:bg-rose-500 text-white font-bold text-sm px-5 py-2.5 rounded-xl transition-all shadow-md shadow-rose-950/20"
              >
              ...
              {tag.isCustom && onDeleteTag && (
                <button
                  onClick={() => {
                    if (confirm('Bạn chắc chắn muốn xóa thẻ tự tạo này khỏi danh sách?')) {
                      onDeleteTag(tag.id);
                    }
                  }}
                  className="inline-flex items-center space-x-1 px-3 py-2 text-xs font-bold text-red-400 bg-red-950/20 hover:bg-red-900/30 border border-red-900/30 rounded-xl transition-colors"
                >
